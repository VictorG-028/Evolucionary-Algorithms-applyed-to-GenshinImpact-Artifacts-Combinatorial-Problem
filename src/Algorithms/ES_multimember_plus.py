...
def create_population(database: list[Artifact], 
    ...
    cromossomes = np.array([], dtype = Build)
...
def mutation(b: Build, bag_of: dict[str, np.ndarray]) -> Build:
    ...
    for gene_type in genes_types: # Faz um pertubaçao em cada gene
        ...
        probability = precomputed_probabiltes[number_diferent_atributes]
...
def ES_multimember_plus(fitness: Callable, 
    ...
    mutation_ = lambda cromossome: mutation(cromossome, bag_of_artifacts)
    ...
    generation_fitness = np.zeros(shape=[MU + LAMBDA]) # Muda o formato da matriz a partir dessa linha
    ...
    while True:
        ...
        new_cromossomes = list(map(mutation_, new_cromossomes))
        ...
        # Calcula o fitness da geração atual
        generation_fitness = vectorized_fitness(selection["Cromossomes"])
        generation_best_fitness = np.max(generation_fitness)
        if generation_best_fitness > best_fitness_so_far:
            best_fitness_so_far = generation_best_fitness
            best_index = np.argmax(generation_fitness)
            best_individual = selection["Cromossomes"][best_index]
            no_change_count = 0
